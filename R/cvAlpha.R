...
#' @name cvAlpha.gamlr
#' @export
cvAlpha.gamlr <- function(x, ...)
  UseMethod("cvAlpha.gamlr")

#' Do elastic net cross-validation for alpha and lambda simultaneously
#'
#' @param x A matrix of predictor variables; or for the plotting methods, an object returned by \code{cvAlpha.gamlr}.
...
#' @rdname cvAlpha.gamlr
#' @method cvAlpha.gamlr default
#' @export
cvAlpha.gamlr.default <- function(x, y, alpha=seq(0, 1, len=11)^3, nfolds=10, ..., outerParallel=NULL,
                                   checkInnerParallel=TRUE)
{
  .cvfunc <- function(a, xmat, y, nfolds, foldid, ...)
  {
    require(gamlr)
    cv.gamlr(x, y, alpha=a, nfolds=nfolds, foldid=foldid, ...)
  }
...
#' }
#' @rdname cvAlpha.gamlr
#' @method cvAlpha.gamlr formula
#' @export
cvAlpha.gamlr.formula <- function(formula, data, ..., weights, offset=NULL, subset=NULL, na.action=getOption("na.action"),
                                   drop.unused.levels=FALSE, xlev=NULL, sparse=FALSE)
{
  ...
  model <- cvAlpha.gamlr.default(x, y, weights=weights, offset=offset, ...)
  model$call <- match.call()
  model$terms <- terms(mf)
  model$sparse <- sparse
  model$na.action <- attr(mf, "na.action")
  class(model) <- c("cvAlpha.gamlr.formula", class(model))
  model
}


#' @param object For the \code{predict} and \code{coef} methods, an object returned by \code{cvAlpha.gamlr}.
...
#' @param ... Further arguments to be passed to lower-level functions. In the case of \code{cvAlpha.gamlr}, these arguments are passed to \code{cv.gamlr}; for \code{predict} and \code{coef}, they are passed to \code{predict.cv.gamlr}; and for \code{plot} and \code{plotLambda}, to \code{plot}.
#'
#' @details
...
#'
#' @method predict cvAlpha.gamlr
#' @rdname cvAlpha.gamlr
#' @export
predict.cvAlpha.gamlr <- function(object, newx, alpha, which=match(TRUE, abs(object$alpha - alpha) < 1e-8), ...)
{
  if(is.na(which))
...
#'
#' @method predict cvAlpha.gamlr.formula
#' @rdname cvAlpha.gamlr
#' @export
predict.cvAlpha.gamlr.formula <- function(object, newdata, alpha, which=match(TRUE, abs(object$alpha - alpha) < 1e-8),
                                           na.action=na.pass, ...)
{
  ...
  predict.cvAlpha.gamlr(object, newx, which=which, ...)
}
...
#'
#' @method coef cvAlpha.gamlr
#' @rdname cvAlpha.gamlr
#' @export
coef.cvAlpha.gamlr <- function(object, alpha, which=match(TRUE, abs(object$alpha - alpha) < 1e-8), ...)
{
  if(is.na(which))
    stop("supplied alpha value not found")
  mod <- object$modlist[[which]]
...
#' @method plot cvAlpha.gamlr
#' @rdname cvAlpha.gamlr
#' @export
plot.cvAlpha.gamlr <- function(x, ...)
{
  n <- length(x$modlist)
...
{
  if(!inherits(x, "cvAlpha.gamlr"))
    stop("not a cvAlpha.gamlr object")
  alpha <- x$alpha
