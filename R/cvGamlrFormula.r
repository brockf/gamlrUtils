...
#' @rdname cv.gamlr
#' @method cv.gamlr default
#' @export
cv.gamlr.default <- function(x, ...)
gamlr::cv.gamlr(x, ...)
...
#' @param ... For \code{cv.gamlr.formula} and \code{cv.gamlr.default}, other arguments to be passed to \code{\link[gamlr:cv.gamlr]{gamlr::cv.gamlr}}; for the \code{predict} and \code{coef} methods, arguments to be passed to their counterparts in package \code{gamlr}.
#'
#' @details
...
#' }
#' @rdname cv.gamlr
#' @method cv.gamlr formula
#' @export
cv.gamlr.formula <- function(formula, data, ..., weights, offset=NULL, subset=NULL, na.action=getOption("na.action"),
                              drop.unused.levels=FALSE, xlev=NULL, sparse=FALSE)
{
    ...
    model <- gamlr::cv.gamlr(x, y, weights=weights, offset=offset, ...)
    model$call <- match.call()
    model$terms <- terms(mf)
    model$sparse <- sparse
    model$na.action <- attr(mf, "na.action")
    class(model) <- c("cv.gamlr.formula", class(model))
    model
}


#' @param object For the \code{predict} and \code{coef} methods, an object of class \code{cv.gamlr.formula}.
#' @param newdata For the \code{predict} method, a data frame containing the observations for which to calculate predictions.
#' @rdname cv.gamlr
#' @method predict cv.gamlr.formula
#' @export
predict.cv.gamlr.formula <- function(object, newdata, na.action=na.pass, ...)
{
    if(!inherits(object, "cv.gamlr.formula"))
        stop("invalid cv.gamlr.formula object")
...
#' @rdname cv.gamlr
#' @method coef cv.gamlr.formula
#' @export
coef.cv.gamlr.formula <- function(object, ...)
{
    if(!inherits(object, "cv.gamlr.formula"))
        stop("invalid cv.gamlr.formula object")
    class(object) <- class(object)[-1]
